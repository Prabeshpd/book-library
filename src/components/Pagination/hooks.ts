...
function handlePageChange(state: any, action: string, totalPageCount: number) {
  switch (action) {
    ...
    case PAGINATION_CHANGE.NEXT_PAGE:
      return state.currentPage < totalPageCount ? { ...state, currentPage: state.currentPage + 1 } : { ...state };
  ...
  const [pageState, dispatch] = useReducer(
    (state: any, action: any) => {
      switch (action.type) {
        ...
        case PAGINATION_CHANGE.PREV_PAGE:
          return handlePageChange(state, (action.state = PAGINATION_CHANGE.PREV_PAGE), totalPageCount);
        case PAGINATION_CHANGE.NEXT_PAGE:
          return handlePageChange(state, (action.state = PAGINATION_CHANGE.NEXT_PAGE), totalPageCount);
        default:
          return state;
      }
    },
